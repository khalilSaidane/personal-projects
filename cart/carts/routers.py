...
class CartItemView:
    ...
    @cart_item_router.put("/{cart_id}/item/{cart_item_id}", response_model=CartItem)
    def update_cart_item_quantity(
            self,
            cart_id: int,
            cart_item_id: int,
            quantity: int = Query(..., ge=0, description="Quantity must be greater than 0"),
    ):
        return self.cart_item_service.update_cart_item(cart_id, cart_item_id, quantity)
